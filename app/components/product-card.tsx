    ...
    <article className={`group relative overflow-hidden rounded-2xl border border-[#063f5b]/8 bg-white shadow-[0_16px_36px_-28px_rgba(6,63,91,.45)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_42px_-26px_rgba(6,63,91,.35)] sm:rounded-[2rem]`}>
      ...
      <div className={compact ? "p-3 sm:p-5" : "p-3 sm:p-6"}>
        ...
        <div className={`border-t border-[#063f5b]/8 ${compact ? "mt-3 pt-3 sm:mt-4 sm:pt-4" : "mt-3 pt-3 sm:mt-5 sm:pt-5"}`}>
          ...
          {hasLiveOffers ? (
            <div className={`flex flex-wrap ${compact ? "mt-2 gap-1" : "mt-2 gap-1 sm:mt-3 sm:gap-2"}`}>
              {merchantNames.map((merchantName) => (
                <span key={merchantName} className={`rounded-full bg-[#f1fbfe] font-bold text-[#063f5b] ${compact ? "px-2 py-1 text-[9px] sm:text-[11px]" : "px-2 py-1 text-[9px] sm:px-3 sm:py-1.5 sm:text-xs"}`}>
                  {merchantName}
                </span>
              ))}
            </div>
          ) : (
            <p className={`${compact ? "mt-2 text-xs" : "mt-2 text-xs sm:mt-3 sm:text-sm"} font-semibold text-[#063f5b]/55`}>Offers temporarily unavailable</p>
          )}
          <span className={`inline-flex items-center gap-2 font-extrabold text-[#009dcc] transition-colors group-hover:text-[#0784b0] ${compact ? "mt-3 text-xs sm:mt-4 sm:text-sm" : "mt-3 text-xs sm:mt-5 sm:text-sm"}`}>
