...
export function GuideCard({ guide, variant = "card" }: { guide: Guide; variant?: "card" | "related" }) {
  return <article className="group relative overflow-hidden rounded-[2rem] border border-[#063f5b]/8 bg-white shadow-[0_16px_36px_-28px_rgba(6,63,91,.4)]">
    ...
    <div className="p-6">
      ...
      {guide.status === "published" ? <Link href={`/guides/${guide.slug}`} className="mt-5 inline-flex items-center gap-2 text-xs font-extrabold text-[#009dcc] transition hover:text-[#0784b0]">Read the guide <span aria-hidden="true">&rarr;</span></Link> : <p className="mt-5 text-xs font-extrabold text-[#063f5b]/45">In preparation</p>}
    </div>
  </article>;
}
