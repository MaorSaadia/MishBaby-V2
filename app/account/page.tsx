...
export default async function AccountPage({ searchParams }: PageProps<"/account">) {
  if (!isSupabaseConfigured) return <section className="mx-auto max-w-3xl px-4 py-12 sm:px-8 sm:py-16"><h1 className="font-display text-3xl font-semibold sm:text-4xl">Accounts are being configured</h1><p className="mt-4 text-[#063f5b]/65">Please check back soon.</p></section>;
  ...
  return <section className="bg-[#f7fcfe] px-4 py-10 sm:px-8 sm:py-20"><div className="mx-auto max-w-6xl">
    <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[#009dcc]">Your MishBaby account</p>
    <h1 className="mt-2 font-display text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">Account</h1>
    ...
    <div className="mt-7 rounded-3xl border border-[#063f5b]/10 bg-white p-5 sm:mt-8 sm:rounded-[2rem] sm:p-8">
      <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#009dcc]">Saved products</p>
      <h2 className="mt-2 font-display text-2xl font-semibold tracking-[-0.04em] sm:text-3xl">Your favorites</h2>
      {favoriteProductResult.error ? <p role="status" className="mt-5 rounded-xl bg-[#fff0f1] px-4 py-3 text-sm text-[#8a2430]">Saved products are temporarily unavailable. Please try again later.</p> : favoriteProducts.length > 0 ? <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-7 sm:gap-5 lg:grid-cols-3">{favoriteProducts.map((product) => <ProductCard key={product.id} product={product} variant="compact" />)}</div> : <div className="mt-5 rounded-2xl bg-[#f1fbfe] p-4 sm:p-5"><p className="font-bold">Nothing saved yet</p><p className="mt-1 text-sm leading-6 text-[#063f5b]/60">Use the heart button on a product page to keep useful finds here.</p></div>}
    </div>
    <div className="mt-5 rounded-3xl border border-[#063f5b]/10 bg-white p-5 sm:mt-6 sm:rounded-[2rem] sm:p-8">
      <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#009dcc]">Saved guides</p>
      <h2 className="mt-2 font-display text-2xl font-semibold tracking-[-0.04em] sm:text-3xl">Your reading list</h2>
      {favoriteGuideResult.error ? <p role="status" className="mt-5 rounded-xl bg-[#fff0f1] px-4 py-3 text-sm text-[#8a2430]">Saved guides are temporarily unavailable. Please try again later.</p> : favoriteGuides.length > 0 ? <div className="mt-6 grid gap-4 sm:mt-7 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">{favoriteGuides.map((guide) => <GuideCard key={guide.id} guide={guide} variant="related" />)}</div> : <div className="mt-5 rounded-2xl bg-[#f1fbfe] p-4 sm:p-5"><p className="font-bold">Your reading list is empty</p><p className="mt-1 text-sm leading-6 text-[#063f5b]/60">Use the bookmark button on a guide to save it here.</p></div>}
    </div>
    <div className="mt-5 rounded-3xl border border-[#063f5b]/10 bg-white p-5 sm:mt-6 sm:rounded-[2rem] sm:p-8">
      <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#009dcc]">Email preferences</p>
      <h2 className="mt-2 font-display text-2xl font-semibold tracking-[-0.04em] sm:text-3xl">Optional MishBaby updates</h2>
    ...
    <div className="mt-5 grid gap-5 sm:mt-6 sm:gap-6 md:grid-cols-2">
      <div className="rounded-3xl border border-[#063f5b]/10 bg-white p-5 sm:rounded-[2rem] sm:p-8"><h2 className="text-xl font-extrabold">Sign-in email</h2><p className="mt-2 break-all text-sm leading-6 text-[#063f5b]/65 sm:text-base">{user.email}</p><SignOutButton /></div>
      <div className="rounded-3xl border border-[#9f2734]/20 bg-white p-5 sm:rounded-[2rem] sm:p-8"><h2 className="text-xl font-extrabold text-[#8a2430]">Delete account</h2><p className="mt-2 text-sm leading-6 text-[#063f5b]/65">This permanently deletes your MishBaby account, saved products, and saved guides and cannot be undone.</p><DeleteAccountForm email={user.email} /></div>
    </div>
  </div></section>;
}
