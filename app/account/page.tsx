...
export default async function AccountPage({ searchParams }: PageProps<"/account">) {
  ...
  const [params, favoriteResult, products] = await Promise.all([
    searchParams,
    getFavoriteProductIds(user.id),
    getPublishedProducts(),
  ]);
  const productsById = new Map(products.map((product) => [product.id, product]));
  const favoriteProducts = favoriteResult.productIds.flatMap((id) => {
  ...
  return <section className="bg-[#f7fcfe] px-5 py-14 sm:px-8 sm:py-20"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-6 grid gap-6 md:grid-cols-2">
      ...
      <div className="rounded-[2rem] border border-[#9f2734]/20 bg-white p-6 sm:p-8"><h2 className="text-xl font-extrabold text-[#8a2430]">Delete account</h2><p className="mt-2 text-sm leading-6 text-[#063f5b]/65">This permanently deletes your MishBaby account and saved products and cannot be undone.</p><DeleteAccountForm email={user.email} /></div>
    </div>
  </div></section>;
}
