...
export default async function AccountPage({ searchParams }: PageProps<"/account">) {
  ...
  return <section className="bg-[#f7fcfe] px-4 py-10 sm:px-8 sm:py-20"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-7 rounded-3xl border border-[#063f5b]/10 bg-white p-5 sm:mt-8 sm:rounded-[2rem] sm:p-8">
      ...
      {favoriteProductResult.error ? <p role="status" className="mt-5 rounded-xl bg-[#fff0f1] px-4 py-3 text-sm text-[#8a2430]">Saved products are temporarily unavailable. Please try again later.</p> : favoriteProducts.length > 0 ? <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-7 sm:gap-5 lg:grid-cols-4">{favoriteProducts.map((product) => <ProductCard key={product.id} product={product} variant="compact" />)}</div> : <div className="mt-5 rounded-2xl bg-[#f1fbfe] p-4 sm:p-5"><p className="font-bold">Nothing saved yet</p><p className="mt-1 text-sm leading-6 text-[#063f5b]/60">Use the heart button on a product page to keep useful finds here.</p></div>}
    </div>
