      ...
      {structuredData && (
        ...
        <section className="relative isolate overflow-hidden border-b border-[#063f5b]/8 bg-[#f1fbfe] px-5 py-8 sm:px-8 sm:py-10">
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <h1 className="mt-2 font-display text-4xl font-semibold leading-tight tracking-[-0.05em] text-[#063f5b] sm:text-5xl">Product categories</h1>
            <p className="mt-3 max-w-2xl text-base leading-7 text-[#063f5b]/70 sm:text-lg">Browse baby essentials, everyday helpers, and useful finds by category.</p>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-5 py-6 sm:px-8 md:py-8">
          <div className="grid gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
            {categories.map((category) => (
              <Link key={category.slug} href={`/categories/${category.slug}`} className="group relative min-h-56 overflow-hidden rounded-3xl border border-[#063f5b]/8 bg-white p-5 shadow-[0_16px_36px_-28px_rgba(6,63,91,.45)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_42px_-26px_rgba(6,63,91,.35)] sm:min-h-64 sm:rounded-[2rem] sm:p-6">
                <div className={`absolute -right-7 -top-7 grid size-28 place-items-center rounded-full ${getCategoryThemeClass(category.colorTheme)} text-3xl text-[#009dcc] transition-transform duration-300 group-hover:scale-110 sm:-right-8 sm:-top-8 sm:size-32 sm:text-4xl`} aria-hidden="true">{category.symbol}</div>
                <div className="relative flex h-full flex-col justify-end">
                  <span className="text-xs font-extrabold uppercase tracking-[0.14em] text-[#009dcc]">Discover</span>
                  <h2 className="mt-2 text-xl font-extrabold tracking-[-0.035em] text-[#063f5b]">{category.name}</h2>
        ...
        <section className="mx-auto max-w-6xl px-5 pb-12 pt-2 sm:px-8 sm:pb-16 sm:pt-4">
